...
def test_bool() -> None:
    ...
    assert not BigMoney.of(USD, ZERO, today)
...
def test_equality() -> None:
    ...
    assert BigMoney.of(USD, ONE, today) == BigMoney.of(USD, ONE.quantize(Decimal("0.0000")), today)
    ## TODO: The following shouldn't be equal.
...
def test_abs() -> None:
    ...
    assert abs(Money.of(USD, -ONE, today)) == Money.of(USD, ONE, today)
    assert abs(BigMoney.of(USD, ZERO, today)) == BigMoney.of(USD, ZERO, today)
    assert abs(BigMoney.of(USD, ONE, today)) == BigMoney.of(USD, ONE, today)
    assert abs(BigMoney.of(USD, -ONE, today)) == BigMoney.of(USD, ONE, today)
...
def test_multiply() -> None:
    ...
    assert B_USD_1_T0 * Decimal("1.5") == B_USD_1_T0.with_qty(Decimal("1.5"))
    assert B_USD_1_T0 * Decimal("1.555") == B_USD_1_T0.with_qty(Decimal("1.555"))
    assert B_USD_1_T0 * Decimal("1.565") == B_USD_1_T0.with_qty(Decimal("1.565"))

    assert NoneMoney() * 1000 == NoneMoney()
    assert M_USD_1_T0 * 1 == M_USD_1_T0
    assert M_USD_1_T0 * Decimal("1.5") == M_USD_1_T0.with_qty(Decimal("1.5"))
    assert M_USD_1_T0 * Decimal("1.555") == M_USD_1_T0.with_qty(Decimal("1.56"))
    assert M_USD_1_T0 * Decimal("1.565") == M_USD_1_T0.with_qty(Decimal("1.56"))
...
def test_truediv() -> None:
    ...
    assert B_USD_10_T0 / Decimal("2.00") == B_USD_10_T0.with_qty(Decimal("5.00"))
    assert B_USD_10_T0 / Decimal("2.50") == B_USD_10_T0.with_qty(Decimal("4.00"))
    assert almost_eq((B_USD_10_T0 / Decimal("3.00")).qty, B_USD_10_T0.with_qty(Decimal("3.333333333333")).qty)

    assert NoneMoney() * 1000 == NoneMoney()
    assert M_USD_10_T0 / 1 == M_USD_10_T0
    assert M_USD_10_T0 / Decimal("2.00") == M_USD_10_T0.with_qty(Decimal("5.00"))
    assert M_USD_10_T0 / Decimal("2.50") == M_USD_10_T0.with_qty(Decimal("4.00"))
    assert M_USD_10_T0 / Decimal("3.00") == M_USD_10_T0.with_qty(Decimal("3.33"))
...
def test_floordiv() -> None:
    ...
    assert NoneBigMoney() // 1000 == NoneBigMoney()
    assert B_USD_10_T0 // 1 == B_USD_10_T0
    assert B_USD_10_T0 // Decimal("2.00") == B_USD_10_T0.with_qty(Decimal("5"))
    assert B_USD_10_T0 // Decimal("2.50") == B_USD_10_T0.with_qty(Decimal("4"))
    assert B_USD_10_T0 // Decimal("3.00") == B_USD_10_T0.with_qty(Decimal("3"))

    assert NoneMoney() * 1000 == NoneMoney()
    assert M_USD_10_T0 // 1 == M_USD_10_T0
    assert M_USD_10_T0 // Decimal("2.00") == M_USD_10_T0.with_qty(Decimal("5"))
    assert M_USD_10_T0 // Decimal("2.50") == M_USD_10_T0.with_qty(Decimal("4"))
    assert M_USD_10_T0 // Decimal("3.00") == M_USD_10_T0.with_qty(Decimal("3"))
